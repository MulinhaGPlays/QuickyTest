...
namespace QuickyTest.Infra.Services.Mock
{
    public class ProveGeneratorMock : IProveGenerator
    {
        ...
        public async IAsyncEnumerable<ChunkModel> GenerateProveEnumerableAsync()
        {
            ...
            foreach (var result in await File.ReadAllTextAsync(@$"C:\Users\Competidor\Documents\Github\QuickyTest\RevolucaoFrancesaComContexto.txt"))
            {
                _json.Append(result);
                Prova? asyncProva = null;
                if (!String.IsNullOrWhiteSpace(_json.ToString().LastOrDefault().ToString()))
                {
